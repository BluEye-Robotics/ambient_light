/*
        The following code has been modified to run on arduino uno
*/
...
#include <stdio.h>
#include <math.h>
#include <avr/pgmspace.h> // For arduino RAM optimization
#include <Arduino.h>
                     ...
                     float *x, float *y, float *z)
{
    ...
    */
    ...
    for (i = 0, lambda = 380; lambda < 780.1; i++, lambda += 5) {
        ...
        X += Me * pgm_read_float_near(&(cie_colour_match[i][0]));
        Y += Me * pgm_read_float_near(&(cie_colour_match[i][1]));
        Z += Me * pgm_read_float_near(&(cie_colour_match[i][2]));
    }
    XYZ = (X + Y + Z);
    *x = X / XYZ;
    *y = Y / XYZ;
